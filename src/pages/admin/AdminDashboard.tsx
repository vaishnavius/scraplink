...
export function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'listings' | 'users' | 'transactions' | 'requests' | 'activity'>('overview');
    ...
    try {
      ...
      recentListings?.forEach(listing => {
        activities.push({
          ...
          description: `${listing.scrap_type} scrap listed (${listing.weight}kg)`,
      ...
      const { data: recentRequests } = await supabase
        ...
        .select(`
          *,
          ...
          scrap_listing:scrap_listings!pickup_requests_scrap_id_fkey (scrap_type)
        `)
        .order('request_date', { ascending: false })
        .limit(10);
      ...
      recentRequests?.forEach(request => {
        activities.push({
          ...
          type: 'pickup_request',
          description: `Pickup request for ${request.scrap_listing?.scrap_type || 'scrap'}`,
      ...
      recentTransactions?.forEach(transaction => {
        activities.push({
          ...
          description: `Transaction completed: ${transaction.scrap_listing?.scrap_type || 'scrap'} (₹${transaction.final_price})`,
  ...
  const filteredTransactions = transactions.filter(transaction => {
    const matchesSearch = transaction.scrap_listing?.scrap_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         transaction.seller?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         transaction.recycler?.name.toLowerCase().includes(searchTerm.toLowerCase());
    return matchesSearch;
  });

  const filteredRequests = pickupRequests.filter(request => {
    const matchesSearch = request.scrap_listing?.scrap_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         request.seller?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         request.recycler?.name.toLowerCase().includes(searchTerm.toLowerCase());
    return matchesSearch;
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8 px-6">
              {[
                ...
                { key: 'listings', label: 'All Scrap Listings', icon: Package },
                { key: 'users', label: 'User Management', icon: Users },
                { key: 'transactions', label: 'All Transactions', icon: DollarSign },
                { key: 'requests', label: 'Pickup Requests', icon: Truck },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                    ...
                    <span className="text-2xl font-bold text-blue-600">{stats.todayLogins}</span>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Recyclers</span>
                    <div className="flex items-center">
                      <div className="w-32 bg-gray-200 rounded-full h-2 mr-3">
                        <div 
                          className="bg-green-600 h-2 rounded-full" 
                          style={{ width: `${stats.totalUsers > 0 ? (stats.totalRecyclers / stats.totalUsers) * 100 : 0}%` }}
                        />
                      </div>
                      <span className="font-medium">{stats.totalRecyclers}</span>
                    </div>
                  </div>

                  <div className="mt-6 p-4 bg-purple-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-purple-900">This Week's Activity</span>
                      <span className="text-2xl font-bold text-purple-600">{stats.thisWeekActivity}</span>
                    </div>
                    <p className="text-sm text-purple-600 mt-1">Total actions (listings, requests, transactions)</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* All other tabs with search functionality */}
        {(activeTab === 'listings' || activeTab === 'users' || activeTab === 'transactions' || activeTab === 'requests') && (
